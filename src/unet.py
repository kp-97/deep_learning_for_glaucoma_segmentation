#https://github.com/cosmic-cortex/pytorch-UNet/blob/master/unet/blocks.py
import torch
...
class First2D(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, dropout=False, skip=False, batch_norm=False):
        ...
        self.skip=False
        if skip:
            self.skip = nn.Sequential(nn.Conv2d(in_channels, out_channels, 1, padding = 0))

    def forward(self, x):
        if self.skip:
            x = self.first(x) + self.skip(x)
...
class Encoder2D(nn.Module):
    ...
    def forward(self, x):
        if self.skip:
            x = self.encoder[0](x)
            x = self.encoder[1:](x) + self.skip(x)
...
class Center2D(nn.Module):
    ...
    def forward(self, x):
        if self.skip:
            x = self.center[0](x)
            x = self.center[1:-1](x) + self.skip(x)
...
class Decoder2D(nn.Module):
    ...
    def forward(self, x):
        if self.skip:
            x = self.decoder[:-1](x) + self.skip(x)
...
class Last2D(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, skip=False, softmax=False, batch_norm=False):
        ...
        self.skip=False
        if skip:
            self.skip = nn.Sequential(nn.Conv2d(in_channels, middle_channels, 1, padding = 0))
        
        self.last = nn.Sequential(*layers)

    def forward(self, x):
        if self.skip:
            x = self.last[:-1](x) + self.skip(x)
...
class UNet2D(nn.Module):
    def __init__(self, in_channels=1, out_channels=8, conv_depths=(64, 128, 256, 512, 1024), skip=False, batch_norm=False, attention=False):
        ...
        encoder_layers = []
        encoder_layers.append(First2D(in_channels, conv_depths[0], conv_depths[0], skip=skip, batch_norm=batch_norm))
        encoder_layers.extend([Encoder2D(conv_depths[i], conv_depths[i + 1], conv_depths[i + 1], skip=skip, batch_norm=batch_norm)
                               for i in range(len(conv_depths)-2)])

        # defining decoder layers
        decoder_layers = []
        decoder_layers.extend([Decoder2D(2 * conv_depths[i + 1], 2 * conv_depths[i], 2 * conv_depths[i], conv_depths[i], skip=skip, batch_norm=batch_norm)
                               for i in reversed(range(len(conv_depths)-2))])
        decoder_layers.append(Last2D(conv_depths[1], conv_depths[0], out_channels, skip=skip, batch_norm=batch_norm))
